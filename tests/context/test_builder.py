"""Tests for hal.context.builder — ContextBuilder."""
...
def builder(workspace: Path) -> ContextBuilder:
    ...
    with patch("hal.context.builder.SkillsLoader") as mock_skills_cls:
        loader = MagicMock()
...
class TestBootstrapFiles:
    ...
    def test_only_loads_configured_bootstrap_files(self, workspace: Path) -> None:
        """Only BOOTSTRAP_FILES are loaded, no legacy fallback."""
        system_dir = workspace / "system"
        system_dir.mkdir(parents=True, exist_ok=True)
        (system_dir / "SOUL.md").write_text("Be helpful.", encoding="utf-8")
