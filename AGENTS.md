...
- Keep frontend tests under `web/tests/`, not `web/scripts/`. Use `web/tests/unit/` for Vitest component/unit tests and `web/tests/regressions/` for bundled node-side regression coverage of core state or adapter logic.
- When adding or fixing frontend behavior, prefer the smallest test layer that can catch the regression:
  pure logic in Vitest unit tests first, bundled regression tests for store/adapter flows that are awkward to mount in jsdom, and browser-level checks only for key end-to-end flows.
...
- `cd web && npm test`: run the full frontend test suite.
- `cd web && npm run test:unit`: run frontend Vitest unit/component tests.
- `cd web && npm run test:regressions`: run frontend bundled regression tests.
- `cd web && npm run build`: verify the frontend production build.
...
- Frontend tests live under `web/tests/`:
  `web/tests/unit/` uses Vitest + Testing Library for component and helper coverage;
  `web/tests/regressions/` uses bundled node-side scripts for higher-friction state and adapter regressions.
- Keep frontend test helpers in `web/tests/` support files so `web/scripts/` stays for development utilities only.
